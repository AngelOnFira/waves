...
func _move_lights(delta):
	if (get_node("Path2D/PathFollow2D").get_offset() > 7750):
		get_node("Path2D/PathFollow2D").set_offset(0)
	else:
		get_node("Path2D/PathFollow2D").set_offset(get_node("Path2D/PathFollow2D").get_offset() + (100 * delta))
